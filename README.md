...
- filterable event-trail evidence at `GET /api/calls/:callId/events` with optional `?type=caller_turn_appended`, `?since=...`, `?offset=0&limit=25`, and `?order=desc` for newest-first lightweight QA and operator audit polling
