...
- lightweight transcript polling at `GET /api/calls/:callId/transcript` with optional `?speaker=caller`, `?since=...&until=...`, `?text=renewal`, `?offset=0&limit=25`, and `?order=desc`
