...
test("GET /reliability serves the guided reliability-lab workflow", async () => {
  ...
  try {
    ...
    assert.deepEqual(payload.labEntryPoint?.actionQueueSummary, {
      ...
      hasRemaining: !payload.labEntryPoint.evidenceSummary.handoffReady,
    ...
    assert.equal(payload.repositoryContracts.stackManifest.values.BROWSER_WEBRTC_BRIDGE_URL, "http://127.0.0.1:8766");
    ...
    assert.ok(payload.repositoryContracts.requiredStackManifestKeys.includes("BROWSER_WEBRTC_BRIDGE_URL"));
  ...
  try {
    await withServer(async (port) => {
      ...
      const combinedBlockers = [
        ...
        ...payload.labEntryPoint.blockingSummary.evidenceBlockers,
      ];
      assert.deepEqual(payload.selectedRunProfile.handoffBlockers, combinedBlockers);
      assert.equal(payload.selectedRunProfile.handoffReady, payload.labEntryPoint.blockingSummary.handoffReady);
      assert.deepEqual(
        payload.selectedTargetMode.handoffChecklist?.map((step) => step.id),
      ...
      assert.deepEqual(payload.selectedTargetMode.handoffBlockers, combinedBlockers);
      assert.equal(payload.selectedTargetMode.handoffReady, payload.labEntryPoint.blockingSummary.handoffReady);
      assert.deepEqual(payload.selectedTargetMode.nextAction, {
...
test("POST /api/demo/run-end-to-end executes a complete usable call flow", async () => {
  await withServer(async (port) => {
    ...
    assert.deepEqual(callerOffsets, [1_000, 5_000, 12_000]);
    const consentEvent = payload.call.events.find((event) => event.type === "customer_consent_recorded");
    assert.equal(Date.parse(consentEvent?.at ?? "") - startedAtMs, 5_000);
    assert.equal(Date.parse(payload.call.operatorSteer.respondedAt ?? "") - startedAtMs, 8_000);
    ...
    assert.equal(payload.proof.summary.transcriptTurns >= 7, true);
  });
});
...
test("retention approval requires its exact pending request and the console follows the concise script", async () => {
  ...
  await withServer(async (port) => {
    ...
    assert.equal(afterModeOverrideCall.pipecatFlow.script.matchedCallerTurns, 2);
    ...
    assert.equal(completedCall?.actionState.scriptedCallerTurnState.progressLabel, "3/3 scripted turns sent");
...
test("delivery-ack caller turns preview without mutating until commit", async () => {
  await withServer(async (port) => {
    ...
    const expectedAgentText = previewPayload.transcript[1].text;
  ...
  await withServer(async (port) => {
    for (const expectedAgentText of [undefined, "", "   ", 42]) {
      ...
      const invalidCommit = await requestJson(port, "POST", `/api/calls/${callId}/caller-turn/commit`, {
        text: "Can you help with billing?",
        conversationMode: "free_caller",
        ...(expectedAgentText === undefined ? {} : { expectedAgentText }),
        timestamp: "2026-06-10T14:00:00.000Z",
      });
      assert.equal(invalidCommit.statusCode, 400);
      assert.equal(
        (invalidCommit.payload as { error: string }).error,
        "caller_turn_commit_expected_agent_text_required",
      );
