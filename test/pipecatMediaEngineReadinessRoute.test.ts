...
test("GET /api/pipecat-media-engine/readiness exposes the shared browser/SIP and FlowManager contract", async () => {
  ...
  try {
    ...
    assert.equal(payload.pipecat17Alignment.issue, "agonza1/agentic-contact-center#222");
    assert.equal(payload.pipecat17Alignment.packageRequirement, "pipecat-ai[webrtc]==1.7.0");
    assert.equal(payload.pipecat17Alignment.primaryTransportTarget, "SmallWebRTCTransport");
    assert.deepEqual(payload.pipecat17Alignment.targetPipeline, [
      "transport.input",
      "rtc-asr STT",
      "Pipecat FlowManager",
    ...
    assert.equal(payload.pipecat17Alignment.browserPrimaryBridge.current, "scripts/pipecat-browser-webrtc-bridge.py");
    assert.equal(payload.pipecat17Alignment.browserPrimaryBridge.legacyFallbackAllowed, false);
    assert.equal(payload.pipecat17Alignment.sipTransportStrategy.transport, "FreeSWITCH Verto/WebRTC agent leg");
    assert.equal(payload.pipecat17Alignment.sipTransportStrategy.sharesPipelineProcessors, true);
    assert.equal(payload.pipecat17Alignment.sipTransportStrategy.processorContractAligned, true);
    assert.equal(payload.pipecat17Alignment.sipTransportStrategy.liveMediaProofComplete, true);
    assert.match(payload.pipecat17Alignment.sipTransportStrategy.preferredRoute, /acc-pipecat/);
    assert.match(payload.pipecat17Alignment.sipTransportStrategy.legacyFallback, /freeswitch-acc-bridge/);
    assert.deepEqual(payload.pipecat17Alignment.sipTransportStrategy.pipelineUnificationDelta, [
    ...
    assert.equal(payload.pipecat17Alignment.flowsDecision.owner, "Pipecat Flows/FlowManager runtime transition adapter");
    assert.equal(payload.pipecat17Alignment.flowsDecision.flowManagerRequiredNow, true);
    assert.equal(payload.pipecat17Alignment.flowsDecision.runtimeImplemented, true);
    ...
    assert.deepEqual(payload.nextUnblockedSlice, {
      ...
      targetContract: "Browser microphone, Pipecat SmallWebRTCTransport, rtc-asr final transcript, selected TTS audio, and browser remote playback are captured in one current-head evidence manifest.",
      verification: "Run the browser WebRTC readiness smoke, capture one local media turn, then require browser-webrtc live proof review readiness.",
      migrationStages: [
        {
          id: "sidecar_contract_ready",
          deliverable: "Confirm ACC, rtc-asr, selected TTS, and Pipecat browser bridge sidecars are reachable.",
          verificationCommand: "npm run browser-webrtc:check -- --url http://127.0.0.1:${PORT:-8026}/health",
        },
        {
          id: "live_media_turn_capture",
          deliverable: "Record a current-head browser media turn with microphone uplink, Pipecat offer/answer, rtc-asr final transcript, TTS audio, and remote playback evidence.",
          verificationCommand: "npm run browser-webrtc:live-proof -- --write-template artifacts/browser-webrtc-live-proof/proof.template.json",
        },
        {
          id: "review_ready_manifest",
          deliverable: "Promote the captured evidence only when the live proof manifest is review-ready.",
          verificationCommand: "npm run browser-webrtc:live-proof -- --evidence artifacts/browser-webrtc-live-proof/proof.json --require-review-ready",
        },
      ],
      acceptance: {
        requiredProof: ["repoHeadEvidence", "browserMicrophoneUplink", "pipecatWebrtcBridge", "rtcAsrFinalTranscript", "ttsAudio", "browserRemoteAudio"],
        retainedAccOwnership: ["operator_controls", "proof_artifacts", "queue_state", "policy_state"],
        rejectedShortcuts: [
          "generated_media_without_browser_capture",
          "stale_repo_head_evidence",
          "rtc_asr_transcript_reused_without_current_media",
          "tts_audio_without_browser_playback",
        ],
      },
    });
    ...
    assert.equal(payload.flowManagerContract.runtimePlan.runtimeAdapter, "pipecat.flows.FlowManager");
...
test("GET /api/pipecat-flowmanager/contract exposes the sidecar-free migration contract", async () => {
  ...
  try {
    ...
    assert.equal(payload.runtimePlan.runtimeAdapter, "pipecat.flows.FlowManager");
