...
async function postMcpSequenceWithSession(
  principalType: string,
  sessionId: string,
  body: unknown,
): Promise<{ statusCode: number; payload: any; headers: Record<string, string | string[] | undefined> }> {
...
test("POST /mcp accepts browser requests from loopback origins", async () => {
  const response = await postMcpWithOrigin("voice_agent", "http://127.0.0.1:5173", {
    jsonrpc: "2.0",
    id: "initialize-loopback-origin",
    method: "initialize",
    params: {
      protocolVersion: "2025-06-18",
      clientInfo: { name: "local-browser-client", version: "1.0.0" },
    },
  });
...
test("POST /mcp rejects browser requests from untrusted origins", async () => {
  const response = await postMcpWithOrigin("operator", "https://attacker.example", {
    jsonrpc: "2.0",
    id: "initialize-untrusted-origin",
    method: "initialize",
    params: {
      protocolVersion: "2025-06-18",
      clientInfo: { name: "untrusted-browser-client", version: "1.0.0" },
    },
  });
...
test("POST /mcp caps retained session state and evicts the oldest session", async () => {
  ...
  try {
    ...
    assert.equal(evictedList.statusCode, 404);
    assert.equal(evictedList.payload.error.message, "ACC MCP session was not found");
  } finally {
    await new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve()));
  }
});

test("POST /mcp returns 404 for unknown MCP session IDs", async () => {
  const unknownSessionResponse = await postMcpSequenceWithSession("voice_agent", "00000000-0000-4000-8000-000000000000", {
    jsonrpc: "2.0",
