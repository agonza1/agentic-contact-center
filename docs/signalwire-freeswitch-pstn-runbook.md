...
- Both modes include active ACL proof from `fs_cli -x 'acl SIGNALWIRE_SOURCE_IP_PROBE SIGNALWIRE_SOURCE_ACL_NAME'` plus a redacted non-provider reject probe; `sourceRestriction.providerOwnedProbe`, `sourceRestriction.activeAclProven`, and `sourceRestriction.activeAclRejectsNonProvider` must all be `true`.
