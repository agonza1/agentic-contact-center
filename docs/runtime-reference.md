...
## Issue #222 shared media-session architecture
...
- Fixture/tester: synthetic caller audio frames and tester-agent turns injected at the Pipeline input, not a parallel proof-only route. `npm run pipecat:fixture:check` remains the sidecar-free contract gate; `python3 scripts/pipecat-fixture-pipeline-smoke.py --input-wav <mono-pcm16.wav>` runs the live in-process fixture source/sink and captures Pipeline output audio when ACC, rtc-asr, and Kokoro are running; omit `--call-id` to start an ACC demo call automatically. `npm run pipecat:tester:scenarios` runs the deterministic tester-agent runner for happy path, barge-in, no-speech timeout, expected transcript mismatch, and transport start failure scenarios. It injects finalized tester `TranscriptionFrame` objects as rtc-asr output into the shared `AccCallerTurnProcessor` -> `KokoroTtsProcessor` boundary, emits transcript/agent response/timing/artifact evidence under `artifacts/pipecat-tester-agent-scenarios/`, and intentionally does not prove live rtc-asr audio recognition or the `/play` fixture audio route.
- SIP/FreeSWITCH: separate telephony transport aligned to the same rtc-asr, ACC caller-turn, and Kokoro processor contract. The Verto/WebRTC leg now has strict local proof for accepted SIP/RTP, same-call rtc-asr final transcript evidence, and non-silent caller-side Kokoro/Pipecat return audio. Do not force SmallWebRTCTransport onto SIP.
...
- `GET /api/voice/sessions/:id/proof`: inspect ownership, pipeline contract, media counters, rtc-asr evidence status, per-stream and cumulative output counters, evaluator blockers, per-gate review readiness with the next required action, and the `correlation` block. The correlation block carries a session-wide `correlationId`, redacted timeline entries for media ingress, STT final/interim evidence, ACC agent processing, TTS playback chunks, barge-in/cancel/flush events, recording/artifact paths, and computed latency metrics such as end-to-end response and interruption latency. Timeline entries intentionally omit transcript text and audio content; use SHA-256/audio byte counts and artifact paths for QA inspection.
...
Issue #213 exposes the current non-SIP browser adapter path for realtime WebRTC into Pipecat. Issue #222 owns keeping this adapter on the shared Pipeline and moving fixture/tester plus SIP transports behind the same processor boundary:
...
Review release requires a real local SIP softphone call with `live_capture`, not the generated self-test. If `RTC_ASR_WS_URL` is not set, the bundle emits an explicit `rtc_asr_blocked` artifact and blocker instead of a fake transcript. Extension `8600` defaults to the local backend-owned Codex OAuth bridge: complete device login from the operator console, then ACC invokes the pinned `gpt-5.4-mini` model without exposing OAuth credentials to browser JavaScript. API-key mode and the local OpenClaw Responses gateway remain explicit alternatives through `ACC_OPENAI_AUTH_MODE=api_key` and `ACC_OPENAI_AUTH_MODE=openclaw_oauth`. Missing login or credentials and bridge/API/model failures fail closed without scripted substitution. Extension `8611` remains the default credential-free local path; use `8612` when intentionally demonstrating scripted failure control.
...
Transfer and takeover are control-plane decisions, not media operations inside ACC. ACC records an auditable JSON command such as `telephony.transfer.requested` with the call id, transfer mode, and target. An integration adapter delivers that command over HTTP, WebSocket, or an event bus and maps the ACC call id to the active media-server channel. FreeSWITCH adapters can execute the request through ESL using `uuid_transfer` or an originate-and-bridge flow. Other SIP systems can use standard `REFER`, or a B2BUA can create a new outbound `INVITE` and bridge the leg. The SIP/media server remains responsible for dialog state and RTP continuity; ACC remains responsible for the bounded policy decision, command, and evidence trail.
