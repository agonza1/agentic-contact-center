...
`actionQueueSummary` condenses the ordered queue into completed, pending, and blocked counts; a `state` of `active`, `blocked`, or `done`; boolean `complete`, `hasRemaining`, `activeRunnable`, `activeBlocked`, `activeRequiresEvidence`, `activeIsFinal`, `followUpRunnable`, `followUpBlocked`, and `followUpComplete` signals; integer `progressPct`; the active action's one-based ordinal, remaining action count, `remainingAfterActive` follow-on count, and `followUpOrdinal`/`followUpStep`/`followUpStatus`/`followUpCommand`/`followUpPurpose`/`followUpEvidence`/`followUpBlockedBy`; plus its step, status, command, purpose, and expected evidence when applicable, so a heartbeat runner can checkpoint the current task without scanning every queue item.
