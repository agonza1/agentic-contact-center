...
`actionQueueSummary` condenses the ordered queue into completed, pending, and blocked counts; the active action's one-based ordinal and remaining action count; plus its step, status, command, purpose, and expected evidence when applicable, so a heartbeat runner can checkpoint the current task without scanning every queue item.
