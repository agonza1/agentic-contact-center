...
def collect_identity_node() -> NodeConfig:
  ...
  <script>
    ...
    function updateAsrRealtimeTranscript(live, text, message) { const nextText = String(text || "").trim(); if (nextText) live.displayText = nextText; if (message.is_final) live.finalText = nextText; return live.displayText; }
    ...
    function handleAsrRealtimeMessage(event) { const live = state.asrLive; if (!live) return; let message; try { message = JSON.parse(event.data); } catch { return; } if (message.type === "ready") { if (live.readyResolve) live.readyResolve(); setAsrLiveStatus("Ready · interim text is a rolling hypothesis; finalization returns the full utterance.", "streaming"); return; } if (message.type === "transcript") { const result = document.getElementById("asr-live-result"); const text = String(message.text || "").trim(); const transcript = updateAsrRealtimeTranscript(live, text, message); const transcribedSeconds = (Number(message.audio_transcribed_ms || 0) / 1000).toFixed(1); const capturedSeconds = (Number(message.audio_received_ms || 0) / 1000).toFixed(1); result.textContent = (message.is_final ? "FINAL · FULL UTTERANCE" : "LIVE · REVISING LAST " + transcribedSeconds + " S") + "\\n" + (transcript || "(No speech detected yet.)"); result.classList.toggle("partial", !message.is_final); setAsrLiveStatus(message.is_final ? "Final transcript · " + capturedSeconds + " s captured" : "Listening · " + capturedSeconds + " s captured · revision " + message.revision, message.is_final ? "transcribed" : "streaming"); if (message.is_final && live.finalResolve) { const resolve = live.finalResolve; live.finalResolve = null; live.finalReject = null; resolve(); } return; } if (message.type === "warning") { setAsrLiveStatus("rtc-asr warning: " + message.message, "streaming"); return; } if (message.type === "error") { const error = new Error(message.message || message.code || "rtc-asr stream failed"); if (live.readyReject) live.readyReject(error); if (live.finalReject) live.finalReject(error); renderAsrRealtimeError(error, live); } }
