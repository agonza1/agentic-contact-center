...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .talk-time { display: inline-flex; align-items: center; width: fit-content; min-height: 28px; padding: 4px 10px; border: 1px solid rgba(36,87,166,.28); border-radius: 999px; background: rgba(36,87,166,.08); color: var(--blue); font-size: 12px; font-weight: 800; letter-spacing: .04em; text-transform: uppercase; }
    .contrast-grid { display: grid; grid-template-columns: minmax(0,1fr) auto minmax(0,1fr); gap: 16px; align-items: stretch; }
    .contrast-card { position: relative; display: grid; gap: 16px; align-content: start; min-height: 390px; padding: 24px; overflow: hidden; border: 1px solid var(--line); border-radius: 18px; background: var(--panel); box-shadow: var(--shadow); }
    ...
    .contrast-list { display: grid; gap: 10px; margin: 0; padding: 0; list-style: none; }
    .contrast-list li { display: grid; grid-template-columns: 84px minmax(0,1fr); gap: 10px; padding: 10px 0; border-top: 1px solid var(--line); }
    .contrast-list strong { color: var(--ink); font-size: 13px; }
    .contrast-list span { color: var(--muted); font-size: 13px; line-height: 1.45; }
    ...
    .machine { display: grid; gap: 14px; padding: 20px; border: 1px solid var(--line); border-radius: 16px; background: var(--panel); box-shadow: var(--shadow); }
    ...
    .clock-tick strong { color: var(--blue); font: 800 12px/1 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; }
    .clock-tick span { color: var(--muted); font-size: 10px; overflow-wrap: anywhere; }
    ...
    .vad-note { color: #94a3b8; font-size: 11px; line-height: 1.45; }
    ...
    .vad-event code { color: #67e8f9; font-size: 10px; }
    .vad-event span { color: #cbd5e1; font-size: 11px; line-height: 1.35; overflow-wrap: anywhere; }
    ...
    .pipecat-node { position: relative; display: grid; gap: 7px; align-content: start; min-height: 162px; padding: 13px; border: 1px solid var(--line); border-radius: 12px; background: rgba(36,87,166,.04); transition: border-color .14s,background .14s,transform .14s,box-shadow .14s; }
    ...
    .pipecat-node code { color: var(--blue); font-size: 10px; line-height: 1.35; overflow-wrap: anywhere; }
    ...
    .pipeline-truth { padding: 10px; border: 1px solid var(--line); border-radius: 9px; color: var(--muted); font-size: 11px; line-height: 1.4; }
    .pipeline-truth strong { display: block; margin-bottom: 3px; color: var(--ink); font-size: 12px; }
...
<body class="${bodyClass}">
  ...
  <main>
    ...
    <section class="section-band slide" data-slide="1" id="realtime-problem"><span class="talk-time">2 minutes</span><span class="kicker">The realtime systems problem</span><h2>Deterministic telephony meets probabilistic inference.</h2><p class="subhead">A phone call expects exact state transitions and continuous media. AI produces estimates whose timing and meaning vary every turn.</p><div class="contrast-grid"><article class="contrast-card contrast-card--telephony"><div class="contrast-head"><strong>Telephony</strong><span class="certainty">STATE IS OBSERVED</span></div><ul class="contrast-list"><li><strong>Protocol</strong><span>INVITE → 200 → ACK → BYE; explicit dialog state.</span></li><li><strong>Media</strong><span>RTP sequence, timestamp, codec, jitter buffer, packet loss.</span></li><li><strong>Failure</strong><span>Timeout, retransmit, reject, transfer, or hang up.</span></li><li><strong>Promise</strong><span>The call must stay alive while everything else is uncertain.</span></li></ul></article><div class="versus">VS</div><article class="contrast-card contrast-card--ai"><div class="contrast-head"><strong>Inference</strong><span class="certainty">MEANING IS ESTIMATED</span></div><ul class="contrast-list"><li><strong>Listening</strong><span>VAD probability and turn-completion prediction.</span></li><li><strong>Language</strong><span>ASR alternatives, partial revisions, model confidence.</span></li><li><strong>Reasoning</strong><span>Variable LLM latency and non-deterministic output.</span></li><li><strong>Risk</strong><span>A plausible response can still be late, wrong, or unsafe.</span></li></ul></article></div><div class="takeaway">The application is the control plane: preserve call state, bound inference, and make uncertainty visible.</div></section>
    <section class="section-band slide" data-slide="2" id="two-machines"><span class="talk-time">3 minutes</span><span class="kicker">Two different machines</span><h2>One runs on clocks. One runs on confidence.</h2><p class="subhead">SIP/RTP state and timing cannot wait for AI to become certain. The bridge needs explicit budgets, cancellation, fallback, and evidence.</p><div class="machine-grid"><article class="machine machine--telephony"><div class="machine-head"><div><strong>SIP / RTP machine</strong><span>Protocol state + packet cadence</span></div><span class="badge ready">bounded</span></div><div class="machine-clock"><div class="clock-tick"><strong>0 ms</strong><span>seq 8041</span></div><div class="clock-tick"><strong>20 ms</strong><span>seq 8042</span></div><div class="clock-tick"><strong>40 ms</strong><span>seq 8043</span></div><div class="clock-tick"><strong>60 ms</strong><span>seq 8044</span></div><div class="clock-tick"><strong>80 ms</strong><span>seq 8045</span></div></div><div class="machine-rules"><div class="machine-rule"><strong>Call state</strong><span>explicit dialog + transaction</span></div><div class="machine-rule"><strong>Audio state</strong><span>continuous, ordered, deadline-driven</span></div><div class="machine-rule"><strong>Recovery</strong><span>retransmit, conceal, reroute, hang up</span></div></div></article><article class="machine machine--ai"><div class="machine-head"><div><strong>AI machine</strong><span>Latency distribution + semantic uncertainty</span></div><span class="badge fixture">variable</span></div><div class="machine-clock"><div class="clock-tick"><strong>~90 ms</strong><span>VAD start</span></div><div class="clock-tick"><strong>~250 ms</strong><span>ASR partial</span></div><div class="clock-tick"><strong>~800 ms</strong><span>ASR final</span></div><div class="clock-tick"><strong>~400 ms</strong><span>LLM first token</span></div><div class="clock-tick"><strong>~250 ms</strong><span>TTS audio</span></div></div><div class="machine-rules"><div class="machine-rule"><strong>Turn state</strong><span>detected, revised, inferred</span></div><div class="machine-rule"><strong>Meaning</strong><span>probabilistic and context-dependent</span></div><div class="machine-rule"><strong>Recovery</strong><span>cancel, retry, constrain, or hand off</span></div></div></article></div><div class="boundary-strip"><div class="boundary-side"><strong>Never block the media clock</strong>Keep RTP and call control independent.</div><div class="boundary-gate">contract boundary</div><div class="boundary-side"><strong>Never hide inference state</strong>Expose partial, final, timeout, cancellation, and fallback.</div></div></section>
    <section class="section-band slide" data-slide="3" id="vad-interruption"><span class="talk-time">Live browser demo</span><span class="kicker">VAD, turns, and interruption</span><h2>The smallest signal changes the whole pipeline.</h2><p class="subhead">Allow the microphone, speak, pause, then play the agent response and interrupt it. This browser energy VAD mirrors the control events; production Pipecat can use model-based VAD and smarter turn strategies.</p><div class="vad-layout"><div class="vad-console"><div class="vad-console-head"><strong>Browser microphone VAD</strong><span class="badge fixture" id="vad-state">mic off</span></div><div class="vad-meter" aria-label="Live microphone level"><span class="vad-meter-fill" id="vad-meter-fill"></span><span class="vad-threshold" id="vad-threshold-line"></span><span class="vad-meter-label" id="vad-level">−∞ dBFS</span></div><div class="vad-slider"><label for="vad-threshold"><span>Speech threshold</span><strong id="vad-threshold-value">−42 dBFS</strong></label><input id="vad-threshold" type="range" min="-60" max="-20" value="-42" step="1"></div><div class="vad-actions"><button class="primary" id="vad-mic" type="button">Start microphone</button><button id="vad-agent" type="button">Play agent; interrupt it</button><button id="vad-simulate" type="button">Simulate barge-in</button><button id="vad-reset" type="button">Reset</button></div><div class="vad-note">Demo tuning: 80 ms speech onset, 350 ms silence hangover, 4 dB hysteresis. VAD detects acoustic activity—not whether the thought is complete.</div><div class="vad-events" id="vad-events" aria-live="polite"><div class="vad-event"><code>READY</code><span>Start the microphone or run the deterministic barge-in simulation.</span></div></div></div><div class="pipeline-panel"><div class="pipeline-panel-head"><strong>Pipecat Pipeline control path</strong><span class="badge ready">interruptions enabled</span></div><div class="pipecat-flow"><div class="pipecat-node" id="vad-node-input"><small>Transport</small><strong>Audio input</strong><code>InputAudioRawFrame</code></div><div class="pipecat-node" id="vad-node-turn"><small>User aggregator</small><strong>VAD + turn start</strong><code>VADUserStartedSpeakingFrame<br>UserStartedSpeakingFrame</code></div><div class="pipecat-node" id="vad-node-stt"><small>rtc-asr</small><strong>Speech → text</strong><code>InterimTranscriptionFrame<br>TranscriptionFrame</code></div><div class="pipecat-node" id="vad-node-agent"><small>ACC</small><strong>Context + policy</strong><code>LLM / tools / operator hold</code></div><div class="pipecat-node" id="vad-node-output"><small>Kokoro + transport</small><strong>Audio output</strong><code>TTSAudioRawFrame<br>InterruptionFrame clears queue</code></div></div><div class="pipeline-truths"><div class="pipeline-truth"><strong>Start is urgent</strong>User speech can cancel queued text and audio immediately.</div><div class="pipeline-truth"><strong>Stop is not “done”</strong>Silence is only an acoustic signal; turn completion may need transcript or Smart Turn.</div><div class="pipeline-truth"><strong>Interruption is state</strong>Cancel downstream work, update context, and preserve what the caller actually heard.</div></div></div></div></section>
  ...
  <script>
    ...
    const state = { slide: 0, slideCount: 9, isPresent: document.body.classList.contains("present"), proof: null, brain: JSON.parse(JSON.stringify(data.brainBlocks)), brainSession: null, asrCapture: null, asrStopping: false, asrModels: [], asrLive: null, vad: null, vadBotSpeaking: false, vadBotTimer: null };
    ...
    function stopVadMic(silent) { const vad = state.vad; if (!vad) return; cancelAnimationFrame(vad.raf); vad.stream.getTracks().forEach(track => track.stop()); vad.source.disconnect(); vad.analyser.disconnect(); vad.context.close().catch(() => undefined); state.vad = null; document.getElementById("vad-mic").textContent = "Start microphone"; renderVadLevel(-Infinity); vadNodes([], state.vadBotSpeaking ? "vad-node-output" : null, null); vadStatus(state.vadBotSpeaking ? "agent speaking" : "mic off", state.vadBotSpeaking ? "ready" : "fixture"); if (!silent) vadLog("MIC_STOPPED", "Browser audio capture released."); }
    async function toggleVadMic() { if (state.vad) { stopVadMic(false); return; } if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) throw new Error("Microphone capture requires HTTPS or localhost."); const AudioContextClass = window.AudioContext || window.webkitAudioContext; if (!AudioContextClass) throw new Error("Web Audio is unavailable in this browser."); const stream = await navigator.mediaDevices.getUserMedia({ audio: { channelCount: 1, echoCancellation: true, noiseSuppression: true, autoGainControl: false }, video: false }); const context = new AudioContextClass(); await context.resume(); const source = context.createMediaStreamSource(stream); const analyser = context.createAnalyser(); analyser.fftSize = 2048; analyser.smoothingTimeConstant = 0.35; source.connect(analyser); state.vad = { stream, context, source, analyser, samples: new Float32Array(analyser.fftSize), raf: 0, speaking: false, speechStartedAt: null, aboveSince: null, belowSince: null }; document.getElementById("vad-mic").textContent = "Stop microphone"; vadStatus("listening", "ready"); vadNodes(["vad-node-input"], state.vadBotSpeaking ? "vad-node-output" : null, null); vadLog("MIC_READY", "Web Audio is measuring RMS locally; no audio leaves the browser."); vadLoop(); }
    function simulateVadBargeIn() { if (state.vadBotSpeaking) stopVadBot(false); startVadBot(); setTimeout(() => vadSpeechStart(performance.now()), 850); setTimeout(() => vadSpeechStop(performance.now()), 1750); }
    function resetVadDemo() { stopVadMic(true); stopVadBot(false); clearTimeout(state.vadBotTimer); state.vadBotTimer = null; document.getElementById("vad-events").innerHTML = '<div class="vad-event"><code>READY</code><span>Start the microphone or run the deterministic barge-in simulation.</span></div>'; document.getElementById("vad-threshold").value = "-42"; updateVadThreshold(); renderVadLevel(-Infinity); vadNodes([], null, null); vadStatus("mic off", "fixture"); }
