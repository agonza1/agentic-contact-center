...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .flow-brand { display: inline-flex; width: fit-content; align-items: center; padding: 9px 15px; border: 1px solid rgba(34, 211, 238, .24); border-radius: 12px; background: linear-gradient(145deg, #07111f, #0a1830); box-shadow: 0 12px 28px rgba(8, 20, 40, .18); }
...
<body class="${bodyClass}">
  ...
  <main>
    ...
    <section class="section-band slide" data-slide="5" id="integration"><span class="kicker">Voice AI integration</span><h2>One Pipecat pipeline. Two realtime transports.</h2><p class="subhead">WebRTC and SIP enter through different media boundaries, then share the same full-duplex voice-agent runtime.</p><div class="transport-integration"><div class="transport-paths"><article class="transport-path transport-path--webrtc"><div class="transport-node"><small>Browser or app</small><strong>WebRTC audio track</strong><code>Opus · DTLS-SRTP</code></div><div class="transport-arrow">↔</div><div class="transport-node"><small>Pipecat transport</small><strong>WebRTC bridge</strong><code>audio track ↔ AudioRawFrame</code></div></article><article class="transport-path transport-path--sip"><div class="transport-node"><small>Phone network</small><strong>SIP + RTP</strong><code>INVITE · PCMU/PCMA</code></div><div class="transport-arrow">↔</div><div class="transport-node"><small>Telephony boundary</small><strong>FreeSWITCH ↔ Pipecat</strong><code>dialog + codec + decoded PCM</code></div></article></div><div class="shared-pipeline" aria-label="Shared Pipecat voice-agent pipeline"><div class="shared-stage"><small>Listen</small><strong>VAD + rtc-asr</strong></div><div class="shared-stage"><small>Orchestrate</small><strong>Pipecat FlowManager</strong></div><div class="shared-stage"><small>Decide</small><strong>ACC state + bounded actions</strong></div><div class="shared-stage"><small>Speak</small><strong>Kokoro + transport output</strong></div></div><div class="integration-truths"><div class="integration-truth"><strong>Transport is an adapter</strong>Browser microphone, prerecorded audio, tester agent, and SIP feed the same processors.</div><div class="integration-truth"><strong>Telephony stays authoritative</strong>FreeSWITCH keeps the SIP dialog, codec negotiation, RTP cadence, transfer, and BYE.</div><div class="integration-truth"><strong>Evidence stays comparable</strong>Every transport emits the same transcript, actions, latency, fallback, and final-state contract.</div></div></div></section>
