...
<body class="${bodyClass}">
  ...
  <script>
    ...
    function asrBenchmarkFor(model) { const profiles = data.asrPanel.benchmarkProfiles || {}; if (!model) return profiles["parakeet-mlx|mlx-community/parakeet-tdt_ctc-110m"] || Object.values(profiles)[0] || null; const key = model.backend + "|" + model.model; const identity = [model.targetId, model.backend, model.model].map(value => String(value || "").toLowerCase()).join("|"); const compatibleFallback = identity.includes("parakeet") && identity.includes("110m") ? profiles["parakeet-mlx|mlx-community/parakeet-tdt_ctc-110m"] : identity.includes("faster-whisper") || identity.includes("whisper") && identity.includes("base.en") ? profiles["faster-whisper|base.en"] : null; return profiles[key] || compatibleFallback || null; }
