...
function buildBasePayload(
  ...
  return {
    ...
    caePanel: {
      ...
      runsPath: "/runs",
      demoRunId: "AgentEvalDemoRun",
      demoRunPath: "/runs/AgentEvalDemoRun",
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .eval-head .eval-head-actions { align-content: end; justify-items: stretch; gap: 8px; min-width: 276px; }
    .eval-demo-run-link { display: inline-flex; align-items: center; justify-content: center; min-height: 46px; padding: 0 18px; border: 1px solid #1d4ed8; border-radius: 9px; background: #2457a6; color: #fff; font-size: 16px; font-weight: 900; line-height: 1.15; text-decoration: none; white-space: nowrap; box-shadow: 0 8px 20px rgba(36,87,166,.2); }
    .eval-demo-run-link:hover { background: #1d4ed8; }
    .eval-demo-run-link:focus-visible { outline: 3px solid rgba(36,87,166,.35); outline-offset: 3px; }
    ...
    .eval-links { display: grid; justify-items: end; gap: 8px; }
    .eval-app-links, .eval-resource-links { display: flex; align-items: center; flex-wrap: wrap; gap: 12px; }
    .eval-app-link { color: var(--blue); font-size: 17px; font-weight: 900; line-height: 1.15; text-decoration-thickness: 2px; text-underline-offset: 4px; }
    .eval-app-link--operator { color: var(--teal); }
    .eval-resource-links a, .eval-resource-links summary { color: var(--blue); font-weight: 750; cursor: pointer; }
    ...
    @media (max-width: 920px) { .slo-layout, .eval-main { grid-template-columns: 1fr; } .slo-bridge { min-height: 46px; } .slo-bridge b { transform: rotate(90deg); } .slo-measures, .eval-progression { grid-template-columns: repeat(2,minmax(0,1fr)); } .eval-step:not(:last-child)::after { display: none; } .eval-head { align-items: flex-start; flex-direction: column; } .eval-head .eval-head-actions { width: 100%; min-width: 0; max-width: 320px; } .eval-summary { grid-template-columns: 1fr; } .eval-evidence-strip, .slo-foot { align-items: flex-start; flex-direction: column; } .eval-links { justify-items: start; } }
...
def collect_identity_node() -> NodeConfig:
  ...
  <script>
    ...
    const slideOrder = ["flow", "voice-evolution", "realtime-problem", "map", "integration", "vad-interruption", "asr-architecture", "asr", "security", "agent", "demo", "tts", "ecosystem", "proof", "slo", "finale"];
    ...
    function renderProofCards() { const checks = ["Account validated before the plan changed", "Price review completed before cancellation", "Cancellation scheduled for August 31"]; document.getElementById("eval-scorecard").innerHTML = checks.map(label => '<div class="eval-check"><i aria-hidden="true">·</i><strong>' + esc(label) + '</strong></div>').join(""); }
