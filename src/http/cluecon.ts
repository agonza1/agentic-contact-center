...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .finale-thesis { display: flex; flex-wrap: wrap; gap: 7px; color: #c4d8ee; font-size: 20px; }
    .finale-thesis strong { color: #67e8f9; }
...
<body class="${bodyClass}">
  ...
  <script>
    ...
    function asrBenchmarkFor(model) { const profiles = data.asrPanel.benchmarkProfiles || {}; if (!model) return profiles["parakeet-mlx|mlx-community/parakeet-tdt_ctc-110m"] || Object.values(profiles)[0] || null; const key = model.backend + "|" + model.model; const targetId = String(model.targetId || "").toLowerCase(); const targetFallback = targetId.includes("faster-whisper") ? profiles["faster-whisper|base.en"] : targetId.includes("parakeet") ? profiles["parakeet-mlx|mlx-community/parakeet-tdt_ctc-110m"] : null; return profiles[key] || targetFallback || null; }
    ...
    function asrRealtimeTokenSequenceMatches(left, right) { return left.length === right.length && left.every((token, index) => asrRealtimeTokenKey(token) === asrRealtimeTokenKey(right[index])); }
    function asrRealtimeTextStartsWith(value, prefix) { const valueTokens = asrRealtimeTokens(value); const prefixTokens = asrRealtimeTokens(prefix); return prefixTokens.length <= valueTokens.length && asrRealtimeTokenSequenceMatches(valueTokens.slice(0, prefixTokens.length), prefixTokens); }
    function asrRealtimeOverlap(left, right) { const leftTokens = asrRealtimeTokens(left); const rightTokens = asrRealtimeTokens(right); const limit = Math.min(leftTokens.length, rightTokens.length); for (let size = limit; size > 0; size -= 1) if (asrRealtimeTokenSequenceMatches(leftTokens.slice(leftTokens.length - size), rightTokens.slice(0, size))) return size; return 0; }
    function appendAsrRealtimeText(base, addition) { const baseTokens = asrRealtimeTokens(base); const additionTokens = asrRealtimeTokens(addition); if (!baseTokens.length) return additionTokens.join(" "); if (!additionTokens.length) return baseTokens.join(" "); if (asrRealtimeTextStartsWith(addition, base)) return additionTokens.join(" "); if (asrRealtimeTextStartsWith(base, addition)) return baseTokens.join(" "); const overlap = asrRealtimeOverlap(base, addition); return baseTokens.concat(additionTokens.slice(overlap)).join(" "); }
    function updateAsrRealtimeTranscript(live, text, message) { const nextText = String(text || "").trim(); if (message.is_final) { live.committedText = nextText; live.partialText = ""; live.displayText = nextText; return live.displayText; } if (!nextText) return live.displayText || appendAsrRealtimeText(live.committedText, live.partialText); const previous = live.partialText || ""; if (!previous || asrRealtimeTextStartsWith(nextText, previous) || asrRealtimeTextStartsWith(previous, nextText)) { live.partialText = nextText; } else { const previousTokens = asrRealtimeTokens(previous); const overlap = asrRealtimeOverlap(previous, nextText); if (overlap > 0) { live.committedText = appendAsrRealtimeText(live.committedText, previousTokens.slice(0, previousTokens.length - overlap).join(" ")); live.partialText = nextText; } else { live.partialText = nextText; } } live.displayText = appendAsrRealtimeText(live.committedText, live.partialText); return live.displayText; }
