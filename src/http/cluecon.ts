...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .voice-origin__photo { position: absolute; z-index: -4; inset: -2%; width: 104%; height: 104%; object-fit: cover; object-position: 50% 38%; filter: saturate(.76) contrast(1.08) brightness(.72); transform: scale(1.015); }
    .voice-origin::before { content: ""; position: absolute; z-index: -3; inset: 0; background: linear-gradient(90deg,rgba(3,7,12,.94) 0%,rgba(3,7,12,.83) 32%,rgba(3,7,12,.57) 52%,rgba(3,7,12,.14) 78%,rgba(3,7,12,.08) 100%),linear-gradient(0deg,rgba(3,7,12,.58),transparent 48%); }
    ...
    .voice-origin__content { display: grid; align-content: center; gap: clamp(15px,2.4vh,28px); width: min(74vw,1180px); min-height: calc(100vh - 62px); padding: clamp(46px,6vw,96px) clamp(24px,2.6vw,46px) clamp(46px,6vw,96px) clamp(46px,6vw,96px); }
    ...
    .voice-origin__fallback { position: relative; max-width: 700px; margin: clamp(5px,1.2vh,14px) 0 0; padding: 8px 0 9px 23px; border-left: 3px solid #fb7185; opacity: 0; transform: translateY(18px); }
    ...
    .state-orchestration { display: grid; grid-template-columns: minmax(360px,1.15fr) minmax(130px,.35fr) minmax(300px,.9fr); gap: 14px; align-items: stretch; }
    .state-core { display: grid; align-content: start; gap: 12px; min-height: 360px; padding: 24px; border: 1px solid rgba(36,87,166,.28); border-left: 5px solid var(--blue); border-radius: 14px; background: linear-gradient(145deg,#fff,rgba(36,87,166,.045)); box-shadow: var(--shadow); }
    .state-core > small, .state-endpoint small { color: var(--muted); font-size: 10px; font-weight: 850; letter-spacing: .09em; text-transform: uppercase; }
    .state-core > strong { font-size: clamp(25px,2.7vw,37px); line-height: 1.05; }
    .state-core > p { max-width: 620px; margin: 0; color: var(--muted); font-size: 13px; line-height: 1.45; }
    .state-core-grid { display: grid; grid-template-columns: repeat(2,minmax(0,1fr)); gap: 9px; margin-top: 4px; }
    .state-core-grid span { display: grid; gap: 4px; min-height: 78px; padding: 12px; border-top: 3px solid var(--state-accent); background: rgba(248,250,252,.9); }
    .state-core-grid span:nth-child(1) { --state-accent: #818cf8; }
    .state-core-grid span:nth-child(2) { --state-accent: #b88723; }
    .state-core-grid span:nth-child(3) { --state-accent: #b4534b; }
    .state-core-grid span:nth-child(4) { --state-accent: #0f766e; }
    .state-core-grid strong { font-size: 13px; }
    .state-core-grid code { color: #475569; font-size: 10.5px; overflow-wrap: anywhere; }
    .state-relations, .state-endpoints { display: grid; grid-template-rows: repeat(4,minmax(72px,1fr)); gap: 9px; }
    .state-relation { display: grid; place-items: center; color: var(--muted); font: 750 10px/1.25 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; text-align: center; }
    .state-relation b { display: block; color: var(--blue); font-size: 24px; line-height: 1; }
    .state-endpoint { display: grid; align-content: center; gap: 5px; min-width: 0; padding: 13px 16px; border: 1px solid var(--line); border-left: 4px solid var(--endpoint-accent); border-radius: 10px; background: var(--panel); box-shadow: var(--shadow); }
    .state-endpoint:nth-child(1) { --endpoint-accent: #818cf8; }
    .state-endpoint:nth-child(2) { --endpoint-accent: #2457a6; }
    .state-endpoint:nth-child(3) { --endpoint-accent: #0f766e; }
    .state-endpoint:nth-child(3)::after { content: "planned · issue #322"; width: fit-content; color: var(--amber); font-size: 9px; font-weight: 800; letter-spacing: .04em; text-transform: uppercase; }
    .state-endpoint:nth-child(4) { --endpoint-accent: #b88723; }
    .state-endpoint strong { font-size: 16px; line-height: 1.12; }
    .state-endpoint code, .state-endpoint span { color: var(--muted); font-size: 10.5px; line-height: 1.35; overflow-wrap: anywhere; }
    .state-principle { display: flex; align-items: center; justify-content: space-between; gap: 18px; padding: 12px 15px; border-top: 1px solid var(--line); color: var(--muted); font-size: 12px; }
    ...
    @media (max-width: 920px) {
      ...
      .voice-origin__content { align-content: end; width: 100%; min-height: calc(100svh - 116px); padding: 30px 24px 38px; }
      .voice-origin__title { max-width: 760px; font-size: clamp(32px,6.4vw,44px); }
      .voice-origin__fallback { max-width: 620px; }
      .state-orchestration { grid-template-columns: 1fr; }
      .state-core { min-height: 0; }
      .state-relations { grid-template-columns: repeat(4,minmax(0,1fr)); grid-template-rows: auto; }
      .state-relation b { transform: rotate(90deg); }
      .state-endpoints { grid-template-rows: repeat(4,minmax(72px,auto)); }
      .state-principle { align-items: flex-start; flex-direction: column; gap: 5px; }
    }
    @media (prefers-reduced-motion: reduce) {
      .voice-origin__photo, .voice-origin__eyebrow, .voice-origin__title span, .voice-origin__title .voice-origin__accent::after, .voice-origin__fallback { animation: none !important; }
      .voice-origin__photo { transform: scale(1.015); }
      .voice-origin__eyebrow, .voice-origin__title span, .voice-origin__fallback { opacity: 1; transform: none; filter: none; }
...
<body class="${bodyClass}">
  ...
  <main>
    ...
    <section class="section-band slide" data-slide="8" id="agent"><span class="kicker">Agent control stack</span><h2>State is the control plane.</h2><p class="subhead">The model proposes. Structured state authorizes. Tools execute. Events reconcile.</p><div class="state-orchestration"><article class="state-core"><small>Deterministic boundary</small><strong>Dynamic call state</strong><p>Pipecat Flow owns the active node. ACC narrows the context, authority, response, and fallback before the model can act.</p><div class="state-core-grid"><span><strong>Flow node</strong><code>cancellation_rescue → policy_hold</code></span><span><strong>Scoped context</strong><code>structured facts + 3 relevant turns</code></span><span><strong>Allowed tools</strong><code>operator.request_approval</code></span><span><strong>Response contract</strong><code>one short response · uncertainty → human</code></span></div></article><div class="state-relations" aria-label="State relationships"><div class="state-relation">state patches<b>→</b></div><div class="state-relation">read / write<b>↔</b></div><div class="state-relation">authorized calls<b>→</b></div><div class="state-relation">events<b>←</b></div></div><div class="state-endpoints"><article class="state-endpoint"><small>Human surface</small><strong>Operator UI</strong><span>approve · transfer · takeover · handoff</span></article><article class="state-endpoint"><small>System of record</small><strong>Structured call state</strong><code>policy_hold · approval_status · final_state</code></article><article class="state-endpoint"><small>Execution boundary</small><strong>Tools + telephony APIs</strong><span>Direct mode or ToolHive + Cedar; consequential actions fail closed.</span></article><article class="state-endpoint"><small>Supervised async work</small><strong>Long-running jobs</strong><code>accepted(jobId) → completed | failed | canceled</code></article></div></div><div class="state-principle"><strong>The LLM is never the system of record.</strong><span>Every tool result returns as an event that updates state before the next response.</span></div></section>
