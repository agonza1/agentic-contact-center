...
function buildBasePayload(
  ...
  return {
    ...
    readiness: [
      ...
      },
      {
        ...
        caveat: pocketTtsProbe?.configured
          ...
          : "Set ACC_TTS_PROVIDER=pocket and POCKET_TTS_BASE_URL to use Pocket-backed streaming TTS through Pipecat.",
      },
      {
        id: "eval",
    ...
    ttsPanel: {
      provider: ttsProvider === "pocket" ? "Pocket TTS via Pipecat" : "Kokoro-82M",
      engine: ttsProvider,
      ...
      pipecatStreaming: {
        ...
        requiredEnv: ["ACC_TTS_PROVIDER=pocket", "POCKET_TTS_BASE_URL", "POCKET_TTS_VOICE"],
      ...
      metricDefinition: "HTTP TTFB stops at first response bytes; playback latency stops only when Pipecat emits first playable audio and the browser emits the playing event.",
      candidates: [
        {
          name: "Kokoro 82M",
      ...
      harness: {
        ...
        summary: "A lightweight, auditable layer routes explicit requests and interaction context to a closed set of style prompts before synthesis.",
        latency: "The paper reports the 4B planner's first style-tool recommendation in under 50 ms; this is planner overhead, not total TTS TTFB.",
      },
    },
    brainBlocks,
    brainPanel: {
...
export async function buildClueConPayloadWithLiveProbes(
  ...
  const liveProbes = await Promise.all([
    ...
    probeHttpSidecar({
      ...
      configuredDetail: "Pocket TTS health probe is reachable for Pipecat streaming TTS readiness.",
      missingDetail: "POCKET_TTS_BASE_URL is not set; Pocket streaming TTS is not active.",
      timeoutMs,
      fetchImpl,
    }),
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .integration-truths { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 9px; }
    .integration-truth { padding: 11px 13px; border: 1px solid var(--line); border-radius: 10px; background: var(--panel); color: var(--muted); font-size: 12px; line-height: 1.4; }
    .integration-truth strong { display: block; margin-bottom: 3px; color: var(--ink); font-size: 13px; }
    ...
    .tts-harness p, .tts-harness span { margin: 0; color: var(--muted); font-size: 11px; line-height: 1.4; }
    .tts-harness a { width: fit-content; font-size: 11px; }
    ...
    .control-stack { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 12px; }
    .control-layer { position: relative; display: grid; align-content: start; gap: 9px; min-height: 220px; padding: 18px; border: 1px solid var(--line); border-radius: 15px; background: var(--panel); box-shadow: var(--shadow); }
    .control-layer:not(:last-child)::after { content: "→"; position: absolute; z-index: 2; right: -21px; top: 50%; transform: translateY(-50%); display: grid; place-items: center; width: 28px; height: 28px; border: 1px solid var(--line); border-radius: 50%; background: var(--bg); color: var(--blue); font-weight: 900; }
    .control-layer--flow { border-top: 4px solid #818cf8; }
    .control-layer--contract { border-top: 4px solid #f59e0b; }
    .control-layer--policy { border-top: 4px solid #10b981; }
    .control-layer small { color: var(--muted); font-size: 10px; font-weight: 850; letter-spacing: .09em; text-transform: uppercase; }
    .control-layer strong { font-size: clamp(18px,2vw,25px); line-height: 1.08; }
    .control-layer p { margin: 0; color: var(--muted); font-size: 13px; line-height: 1.42; }
    .control-list { display: grid; gap: 6px; margin: 0; padding: 0; list-style: none; }
    .control-list li { padding: 7px 9px; border: 1px solid var(--line); border-radius: 8px; background: #f8fafc; color: #334155; font: 650 11px/1.35 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; }
    .control-layer--policy .badge { margin-bottom: 1px; }
    .contract-evidence { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 8px; padding: 12px; border: 1px solid rgba(36,87,166,.22); border-radius: 11px; background: rgba(36,87,166,.05); }
    .contract-evidence span { display: grid; gap: 3px; color: var(--muted); font-size: 10px; }
    .contract-evidence strong { color: var(--ink); font: 750 11px/1.35 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; overflow-wrap: anywhere; }
    .agent-max-impact { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 8px; }
    .agent-impact { padding: 10px 12px; border-left: 3px solid var(--teal); background: rgba(15,118,110,.06); color: var(--muted); font-size: 11px; line-height: 1.4; }
    .agent-impact strong { display: block; color: var(--ink); font-size: 12px; }
    ...
    @media (min-width: 921px) and (max-height: 820px) {
      ...
      .present #vad-interruption .pipeline-truth { padding: 8px; }
...
<body class="${bodyClass}">
  ...
  <main>
    ...
    <section class="section-band slide" data-slide="7" id="tts"><span class="kicker">Live TTS latency lab</span><h2>Measure playback start—not request completion.</h2><p class="subhead">Stream ${escapeHtml(payload.ttsPanel.provider)} into the browser through Pipecat and separate HTTP first-byte latency from the moment audio actually starts playing.</p><div class="tts-layout"><div class="tts-lab"><div class="tts-lab-head"><div><strong>${escapeHtml(payload.ttsPanel.provider)}</strong><span>${escapeHtml(payload.ttsPanel.model)} · ${escapeHtml(payload.ttsPanel.voice)}</span></div><span class="badge ${payload.ttsPanel.status === "streaming_ready" ? "ready" : "fixture"}" id="tts-badge">${payload.ttsPanel.status === "streaming_ready" ? "streaming ready" : "local sidecar required"}</span></div><label for="tts-text">Text to synthesize</label><textarea id="tts-text" rows="4">AI may be probabilistic, but the system around it does not have to be.</textarea><div class="actions"><button class="primary" id="tts-run" type="button">Run ${escapeHtml(payload.ttsPanel.engine === "pocket" ? "Pocket" : "Kokoro")}</button><span class="muted" id="tts-status">${escapeHtml(payload.ttsPanel.metricDefinition)}</span></div><audio id="tts-audio" controls hidden></audio><div class="tts-metrics"><div class="plain metric"><span class="kicker">HTTP TTFB</span><strong id="tts-ttfb">—</strong></div><div class="plain metric"><span class="kicker">Playback</span><strong id="tts-playback">—</strong></div><div class="plain metric"><span class="kicker">Total stream</span><strong id="tts-total">—</strong></div><div class="plain metric"><span class="kicker">Audio</span><strong id="tts-bytes">—</strong></div></div></div><div class="tts-candidates"><div class="tts-candidates-head"><strong>Main OSS recommendations</strong><span>Published conditions—not a universal ranking</span></div>${payload.ttsPanel.candidates.map((candidate) => `<article class="tts-candidate"><span><strong>${escapeHtml(candidate.name)}</strong><b>${escapeHtml(candidate.latency)}</b></span><small>${escapeHtml(candidate.condition)}</small><a href="${candidate.sourceUrl}" target="_blank" rel="noreferrer">${escapeHtml(candidate.sourceLabel)} ↗</a></article>`).join("")}<p>${escapeHtml(payload.ttsPanel.comparisonCaveat)}</p></div></div><aside class="tts-harness"><div><span class="kicker">Expression as a governed layer</span><strong>${escapeHtml(payload.ttsPanel.harness.title)}</strong><p>${escapeHtml(payload.ttsPanel.harness.summary)}</p></div><div><span>${escapeHtml(payload.ttsPanel.harness.latency)}</span><a href="${payload.ttsPanel.harness.sourceUrl}" target="_blank" rel="noreferrer">Paper ↗</a></div></aside></section>
    <section class="section-band slide" data-slide="8" id="agent"><span class="kicker">Agent control stack</span><h2>Deterministic flow. Bounded agent. Policy-gated actions.</h2><p class="subhead">Pipecat controls conversational state. ACC constrains the current decision. Consequential tools can pass through an independent policy gate.</p><div class="control-stack"><article class="control-layer control-layer--flow"><small>1 · Current ACC control</small><strong>Pipecat Flow</strong><p>State and transitions remain outside the model.</p><ul class="control-list"><li>cancellation_rescue</li><li>policy_hold</li><li>continue | human_handoff</li></ul></article><article class="control-layer control-layer--contract"><small>2 · State contract</small><strong>Bounded intelligence</strong><p>Only the active decision receives context and authority.</p><ul class="control-list"><li>context: structured state + 3 relevant turns</li><li>speech: one short response</li><li>agent: request approval; never grant it</li><li>fallback: timeout or uncertainty → human</li></ul></article><article class="control-layer control-layer--policy"><small>3 · Optional production path</small><strong>ToolHive + Cedar</strong><span class="badge fixture">planned · issue #322</span><p>Authenticate the principal, authorize the tool and arguments, then execute or deny outside the model.</p><ul class="control-list"><li>direct mode remains the default</li><li>enabled gateway fails closed</li><li>operator identity applies approved offers</li></ul></article></div><div class="contract-evidence"><span>Flow state<strong>policy_hold</strong></span><span>Agent authority<strong>operator.request_approval</strong></span><span>Gateway mode<strong>direct | toolhive (optional)</strong></span></div><div class="agent-max-impact"><div class="agent-impact"><strong>Small speech context</strong>Load only the active state, structured facts, and relevant turns.</div><div class="agent-impact"><strong>Specialized contracts</strong>Each state or agent gets distinct prompts, tools, skills, output, and eval rules.</div><div class="agent-impact"><strong>Supervised async work</strong>MCP/CLI actions use accepted(jobId) → completed | failed | canceled.</div></div></section>
  ...
  <script>
    ...
    const slideOrder = ["flow", "realtime-problem", "two-machines", "map", "integration", "vad-interruption", "asr", "security", "agent", "demo", "tts", "ecosystem", "proof", "finale"];
    ...
    async function runTtsLab() {
      ...
      const ttsProviderLabel = data.ttsPanel.engine === "pocket" ? "Pocket" : "Kokoro";
      if (!text) { status.textContent = "Enter text before running " + ttsProviderLabel + "."; return; }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(failure.detail || failure.nextStep || failure.error || ttsProviderLabel + " synthesis failed.");
        }
        ...
        const responseProvider = response.headers.get("x-acc-tts-provider") || data.ttsPanel.engine;
        while (true) {
        ...
        if (!bytes || firstByteMs === null) throw new Error(ttsProviderLabel + " returned no audio bytes.");
        ...
        if (playbackMs === null) {
          status.textContent = "HTTP first-byte includes ACC proxy overhead" + (upstreamTtfb ? "; " + responseProvider + "-side first bytes took " + upstreamTtfb + " ms." : ".") + " Waiting for the browser playing event.";
    ...
    function renderOperatorDrill(payload) { const screen = document.getElementById("demo-screen"); const integration = payload.integration || {}; const patterns = Array.isArray(integration.executionPatterns) ? integration.executionPatterns : []; const command = integration.controlSequence || integration.controlMessage; screen.classList.remove("has-transcript"); screen.classList.add("has-drill"); screen.innerHTML = '<div class="drill-summary">' + esc(payload.summary) + '</div><div class="drill-boundary"><strong>Control plane → media plane</strong><span>' + esc(integration.controlPlane || "ACC records a bounded operator action.") + '</span><span>' + esc(integration.mediaPlane || "The configured telephony adapter executes it.") + '</span></div>' + (command ? '<pre class="drill-command">' + esc(JSON.stringify(command, null, 2)) + '</pre>' : '') + (patterns.length ? '<ul class="drill-patterns">' + patterns.map(pattern => '<li>' + esc(pattern) + '</li>').join("") + '</ul>' : '') + (integration.demoCaveat ? '<span class="muted">' + esc(integration.demoCaveat) + '</span>' : ''); }
    ...
    document.getElementById("drill-tool").addEventListener("click", () => runOperatorDrill("tool_timeout").catch(error => { document.getElementById("demo-screen").textContent = String(error.message || error); }));
    document.getElementById("drill-runtime").addEventListener("click", () => runOperatorDrill("runtime_failure").catch(error => { document.getElementById("demo-screen").textContent = String(error.message || error); }));
    document.getElementById("drill-transfer").addEventListener("click", () => runOperatorDrill("transfer").catch(error => { document.getElementById("demo-screen").textContent = String(error.message || error); }));
    document.getElementById("drill-takeover").addEventListener("click", () => runOperatorDrill("takeover").catch(error => { document.getElementById("demo-screen").textContent = String(error.message || error); }));
    document.getElementById("drill-end").addEventListener("click", () => runOperatorDrill("end_call").catch(error => { document.getElementById("demo-screen").textContent = String(error.message || error); }));
    document.getElementById("drill-asr").addEventListener("click", () => runMediaFailureDrill("rtc_asr_unavailable").catch(error => { document.getElementById("demo-screen").textContent = String(error.message || error); }));
    document.getElementById("drill-tts").addEventListener("click", () => runMediaFailureDrill("tts_unavailable").catch(error => { document.getElementById("demo-screen").textContent = String(error.message || error); }));
