...
function buildBasePayload(
  ...
  return {
    ...
    asrPanel: {
      ...
      noiseGuidance: {
        sourceUrl: "https://www.twilio.com/en-us/blog/developers/tutorials/product/handle-background-noise-when-using-conversation-relay-nodejs",
        sourceLabel: "Twilio + Deepgram noisy-call field test",
        findings: [
          "Flux passed the combined vacuum-and-drill condition on its default configuration in the published test.",
          "Nova-3 needed interruptSensitivity=low and ignoreBackchannel=true to avoid false interruptions and backchannel turns.",
          "For Flux turn-finalization errors in exceptional noise, eotThreshold=0.75 is a documented tuning point; keep domain hints explicit.",
        ],
        caveat: "These are ConversationRelay controls, not rtc-asr settings. The transferable lesson is to test recognition, interruption, and end-of-turn behavior together.",
      },
    },
    ttsPanel: {
      provider: "Kokoro-82M",
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .ecosystem-card small { color: var(--muted); font-size: 11px; font-weight: 850; letter-spacing: .1em; text-transform: uppercase; }
    .ecosystem-card strong { font-size: clamp(21px, 2.5vw, 30px); line-height: 1.04; }
    .ecosystem-card span { color: #c5d5e7; line-height: 1.4; }
...
<body class="${bodyClass}">
  ...
  <script>
    ...
    const slideOrder = ["flow", "realtime-problem", "two-machines", "vad-interruption", "asr", "tts", "map", "demo", "agent", "security", "ecosystem", "proof"];
    const asrSlide = document.getElementById("asr");
    const mapSlide = document.getElementById("map");
    if (asrSlide && mapSlide) mapSlide.before(asrSlide);
    const ttsSlide = document.getElementById("tts");
    if (asrSlide && ttsSlide) asrSlide.after(ttsSlide);
    const securitySlide = document.getElementById("security");
    ...
    document.getElementById("preview-brain").addEventListener("click", () => previewBrain().catch(error => { document.getElementById("proof-json").textContent = String(error.message || error); }));
    document.getElementById("apply-brain").addEventListener("click", () => applyBrain().catch(error => { document.getElementById("proof-json").textContent = String(error.message || error); }));
    document.getElementById("reset-brain").addEventListener("click", () => resetBrain().catch(error => { document.getElementById("proof-json").textContent = String(error.message || error); }));
