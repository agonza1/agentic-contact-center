...
function buildBasePayload(
  ...
  return {
    ...
    asrPanel: {
      ...
      benchmarks: [
        { label: "first partial", value: "238 ms", caveat: "example target; open the benchmark site for model-specific artifacts" },
        { label: "finalization", value: "812 ms", caveat: "example target; the live recorder reports its own request latency" },
        { label: "RTF", value: "0.41x", caveat: "example Local STT v1 target; not a claim about the selected live model" },
      ],
    },
    brainBlocks,
    brainPanel: {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .asr-heading { display: flex; flex-wrap: wrap; align-items: end; justify-content: space-between; gap: 12px; }
    .asr-heading h2 { margin-bottom: 0; }
    .asr-live-lab { position: relative; display: grid; gap: 14px; padding: 18px; border: 1px solid rgba(36,87,166,.24); border-radius: 14px; background: linear-gradient(145deg, #07111f, #10233c); color: #e7f5ff; box-shadow: var(--shadow); overflow: hidden; }
    .asr-live-lab::before { content: ""; position: absolute; inset: 0; pointer-events: none; opacity: .14; background-image: linear-gradient(rgba(125,211,252,.18) 1px, transparent 1px), linear-gradient(90deg, rgba(125,211,252,.18) 1px, transparent 1px); background-size: 28px 28px; }
    ...
    .asr-live-head strong { font-size: 18px; }
    .asr-live-controls { display: grid; grid-template-columns: minmax(180px, 1fr) auto; gap: 10px; }
    .asr-live-controls select { min-width: 0; min-height: 42px; padding: 0 10px; border: 1px solid rgba(125,211,252,.32); border-radius: 8px; background: rgba(2,8,20,.82); color: #e7f5ff; font: 700 12px/1.2 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; }
    .asr-live-controls button { min-height: 42px; padding-inline: 16px; }
    .asr-live-wave { height: 54px; display: flex; align-items: center; justify-content: center; gap: 4px; border: 1px solid rgba(125,211,252,.18); border-radius: 10px; background: rgba(2,8,20,.68); color: #38bdf8; }
    .asr-live-wave span { width: 4px; border-radius: 99px; background: currentColor; opacity: .42; transform: scaleY(.5); transform-origin: center; }
    ...
    .asr-live-result { min-height: 106px; margin: 0; padding: 13px; border: 1px solid rgba(125,211,252,.18); border-radius: 10px; background: rgba(2,6,18,.88); color: #d9f7ff; white-space: pre-wrap; overflow-wrap: anywhere; font: 13px/1.5 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; }
    .asr-live-status { color: #9db0c5; font-size: 12px; line-height: 1.4; }
    ...
    @media (max-width: 520px) { #demo .actions { grid-template-columns: minmax(0, 1fr); } #demo .screen { min-height: 240px; } .transcript-turn { grid-template-columns: minmax(0, 1fr); gap: 4px; } }
  </style>
</head>
<body class="${bodyClass}">
  ...
  <main>
    ...
    <section class="section-band slide" data-slide="3" id="asr"><span class="kicker">Live ASR lab</span><div class="asr-heading"><h2>rtc-asr is measurable and swappable.</h2><a class="mode-link" href="${payload.asrPanel.benchmarkUrl}" target="_blank" rel="noreferrer">Open benchmark site ↗</a></div><p class="subhead">Record a short utterance and watch the active local model transcribe it. Model choices map to separately warmed rtc-asr endpoints, so switching is real and does not hide model-load latency.</p><div class="two"><div><div class="asr-live-lab"><div class="asr-live-head"><strong>Microphone → rtc-asr → transcript</strong><span class="badge fixture" id="asr-live-badge">checking sidecar</span></div><div class="asr-live-controls"><label><span class="muted">Active model target</span><select id="asr-model-select" aria-label="rtc-asr model target" disabled><option>Loading models…</option></select></label><button class="primary" id="asr-record" type="button" disabled>Record 6 seconds</button></div><div class="asr-live-wave" id="asr-live-wave" aria-hidden="true"><span style="--wave-i:0;height:12px"></span><span style="--wave-i:1;height:28px"></span><span style="--wave-i:2;height:18px"></span><span style="--wave-i:3;height:42px"></span><span style="--wave-i:4;height:22px"></span><span style="--wave-i:5;height:34px"></span><span style="--wave-i:6;height:16px"></span><span style="--wave-i:7;height:38px"></span><span style="--wave-i:8;height:24px"></span><span style="--wave-i:9;height:46px"></span><span style="--wave-i:10;height:20px"></span><span style="--wave-i:11;height:32px"></span><span style="--wave-i:12;height:14px"></span><span style="--wave-i:13;height:36px"></span><span style="--wave-i:14;height:26px"></span><span style="--wave-i:15;height:40px"></span></div><span class="asr-live-status" id="asr-live-status" aria-live="polite">Waiting for rtc-asr model discovery.</span><pre class="asr-live-result" id="asr-live-result">Your live transcript will appear here.</pre></div><div class="asr-events" id="asr-events"></div></div><div class="grid" id="asr-benchmarks"></div></div></section>
  ...
  <script>
    ...
    const state = { slide: 0, slideCount: 6, isPresent: document.body.classList.contains("present"), proof: null, brain: JSON.parse(JSON.stringify(data.brainBlocks)), brainSession: null, asrCapture: null, asrStopping: false };
    ...
    async function loadAsrModels() { const select = document.getElementById("asr-model-select"); const button = document.getElementById("asr-record"); try { const response = await fetch(data.asrPanel.modelsRoute); const payload = await response.json(); if (!response.ok || !Array.isArray(payload.models)) throw new Error(payload.nextStep || payload.error || "model discovery failed"); select.innerHTML = payload.models.map(model => '<option value="' + esc(model.targetId) + '"' + (model.ready ? '' : ' disabled') + '>' + esc(model.targetLabel + " · " + model.backend + " · " + model.model + (model.ready ? "" : " (unavailable)")) + '</option>').join(""); const active = payload.models.find(model => model.targetId === payload.activeTargetId && model.ready) || payload.models.find(model => model.ready); if (!active) throw new Error("No warmed rtc-asr model target is ready."); select.value = active.targetId; select.disabled = false; button.disabled = false; setAsrLiveStatus("Ready: " + active.backend + " / " + active.model + " (" + active.responseMs + " ms probe).", "ready"); } catch (error) { select.innerHTML = '<option>rtc-asr unavailable</option>'; select.disabled = true; button.disabled = true; setAsrLiveStatus("Live transcription unavailable: " + String(error.message || error), "error"); } }
