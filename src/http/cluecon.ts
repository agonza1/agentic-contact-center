...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .eval-head { display: flex; align-items: end; justify-content: space-between; gap: 20px; }
    .eval-head > div { display: grid; gap: 6px; }
    .eval-head .subhead { max-width: 870px; }
    .eval-head button { flex: 0 0 auto; min-height: 46px; padding-inline: 20px; font-size: 14px; }
    .eval-progression { display: grid; grid-template-columns: repeat(5,minmax(0,1fr)); gap: 20px; }
    .eval-step { position: relative; display: grid; gap: 4px; min-height: 78px; padding: 13px 14px; border: 1px solid var(--line); border-radius: 12px; background: #fff; }
    .eval-step:not(:last-child)::after { content: ""; position: absolute; top: 50%; right: -16px; width: 9px; height: 9px; border-top: 2px solid #5072a7; border-right: 2px solid #5072a7; transform: translateY(-50%) rotate(45deg); }
    .eval-step small { color: var(--muted); font-size: 10px; font-weight: 850; letter-spacing: .08em; text-transform: uppercase; }
    .eval-step strong { font-size: 17px; line-height: 1.15; }
    .eval-step span { color: var(--blue); font-size: 12px; font-weight: 800; }
    .eval-main { display: grid; grid-template-columns: minmax(0,1.12fr) minmax(340px,.88fr); gap: 18px; align-items: stretch; }
    .eval-verdict { display: grid; align-content: center; gap: 7px; min-height: 210px; padding: 24px 28px; border: 1px solid rgba(36,87,166,.28); border-radius: 16px; background: linear-gradient(145deg,#071629,#0b2840); color: #e8f4ff; box-shadow: var(--shadow); }
    .eval-verdict small { color: #67e8f9; font-size: 11px; font-weight: 900; letter-spacing: .12em; text-transform: uppercase; }
    .eval-verdict b { width: fit-content; padding: 5px 10px; border-radius: 999px; background: rgba(125,211,252,.12); color: #bae6fd; font-size: 14px; letter-spacing: .08em; }
    .eval-verdict.pass b { background: rgba(52,211,153,.18); color: #a7f3d0; }
    .eval-verdict.fail b { background: rgba(251,191,36,.18); color: #fde68a; }
    .eval-verdict strong { color: #f8fafc; font-size: clamp(29px,3.5vw,44px); line-height: 1.02; }
    .eval-verdict p { max-width: 680px; margin: 0; color: #bfd0df; font-size: 16px; line-height: 1.4; }
    .eval-checks { display: grid; align-content: center; gap: 9px; padding: 18px; border: 1px solid var(--line); border-radius: 16px; background: #fff; }
    .eval-check { display: grid; grid-template-columns: 28px minmax(0,1fr); gap: 10px; align-items: center; padding: 10px 0; border-bottom: 1px solid var(--line); }
    .eval-check:last-child { border-bottom: 0; }
    .eval-check i { display: grid; place-items: center; width: 26px; height: 26px; border-radius: 50%; background: rgba(100,116,139,.12); color: var(--muted); font-style: normal; font-weight: 900; }
    .eval-check.pass i { background: rgba(15,118,110,.12); color: var(--teal); }
    .eval-check.fail i { background: rgba(180,35,24,.1); color: var(--danger); }
    .eval-check strong { font-size: 14px; line-height: 1.3; }
    .eval-summary { display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 10px; }
    .eval-summary-item { display: grid; gap: 3px; padding: 11px 13px; border-left: 3px solid var(--teal); background: rgba(15,118,110,.06); }
    .eval-summary-item.warning { border-left-color: #d97706; background: rgba(245,158,11,.08); }
    .eval-summary-item small { color: var(--muted); font-size: 10px; font-weight: 850; letter-spacing: .07em; text-transform: uppercase; }
    .eval-summary-item strong { font-size: 15px; }
    .eval-evidence-strip { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding-top: 10px; border-top: 1px solid var(--line); color: var(--muted); font-size: 12px; }
    .eval-links { display: flex; align-items: center; gap: 12px; }
    .eval-links a, .eval-links summary { color: var(--blue); font-weight: 750; cursor: pointer; }
    .eval-details { position: relative; }
    .eval-details[open] { position: absolute; z-index: 30; right: 28px; bottom: 28px; width: min(720px,calc(100vw - 56px)); padding: 14px; border: 1px solid var(--line); border-radius: 12px; background: #fff; box-shadow: 0 22px 60px rgba(15,23,42,.24); }
    .eval-details .proof-pre { margin-top: 10px; min-height: 180px; max-height: 360px; }
    ...
    @media (max-width: 920px) { .slo-layout, .eval-main { grid-template-columns: 1fr; } .slo-bridge { min-height: 46px; } .slo-bridge b { transform: rotate(90deg); } .slo-measures, .eval-progression { grid-template-columns: repeat(2,minmax(0,1fr)); } .eval-step:not(:last-child)::after { display: none; } .eval-head { align-items: flex-start; flex-direction: column; } .eval-summary { grid-template-columns: 1fr; } .eval-evidence-strip, .slo-foot { align-items: flex-start; flex-direction: column; } }
...
<body class="${bodyClass}">
  ...
  <main>
    ...
    <section class="section-band slide" data-slide="5" id="demo"><span class="kicker">Policy-control example</span><h2>A cancellation request, handled clearly.</h2><p class="subhead">Hear the problem → offer one next step → get approval → confirm the caller's choice.</p><div class="demo-shell"><div class="demo-commandbar"><button class="primary" id="run-demo" type="button">Run cancellation scenario</button><div class="demo-drill-picker"><label for="demo-drill-select">Try another control</label><select id="demo-drill-select"><optgroup label="Fail closed"><option value="tool_timeout">Tool timeout</option><option value="runtime_failure">Runtime failure</option><option value="rtc_asr_unavailable">ASR unavailable</option><option value="tts_unavailable">TTS unavailable</option></optgroup><optgroup label="Call control"><option value="transfer">Transfer</option><option value="takeover">Takeover</option><option value="end_call">End call</option></optgroup></select><button id="run-demo-drill" type="button">Run selected drill</button></div></div><div class="demo-control-story" id="demo-control-story" aria-label="Cancellation request control sequence"><div class="demo-control-step" data-step="1"><small>Listen</small><strong>Price went up</strong><span>The caller gives one clear reason.</span></div><div class="demo-control-step" data-step="2"><small>Offer</small><strong>Price review</strong><span>One option, explained in plain language.</span></div><div class="demo-control-step" data-step="3"><small>Approve</small><strong>Supervisor check</strong><span>No promise before approval.</span></div><div class="demo-control-step" data-step="4"><small>Confirm</small><strong>Caller's choice</strong><span>Keep the policy or cancel now.</span></div></div><div class="screen" id="demo-screen"><div class="demo-result-head"><small>Ready</small><strong>The caller chooses. The application authorizes.</strong><p>Run the scenario to see a short, natural conversation with a clear final choice.</p></div></div><details class="demo-evidence" id="demo-evidence"><summary><span><strong>Conversation + audit evidence</strong><small id="demo-evidence-count">Available after the run</small></span><b id="demo-evidence-toggle">Expand</b></summary><div class="demo-evidence-grid"><div><h3>Customer conversation</h3><div class="demo-transcript-detail" id="demo-transcript-detail"></div></div><div><h3>Authoritative events</h3><div class="timeline" id="timeline"></div></div></div></details></div></section>
...
def collect_identity_node() -> NodeConfig:
    ...
    <section class="section-band slide" data-slide="13" id="proof"><div class="eval-head"><div><span class="kicker">ConversationAgentEvals</span><h2>Different voice agents. One evaluation contract.</h2><p class="subhead">ConversationAgentEvals runs the same scenario against web and VoIP/SIP voice agents. A target adapter connects to each agent's call interface and returns its transcript, actions, timing, and outcome in one comparable format.</p></div><button id="run-eval" type="button" class="primary">Run Voice AI target</button></div><div class="eval-progression" aria-label="Cross-target evaluation progression"><div class="eval-step"><small>1 · Evaluation scenario</small><strong>Cancellation rescue</strong><span>repeatable</span></div><div class="eval-step"><small>2 · Connect to agent</small><strong>Web or VoIP</strong><span>adapter translates call + evidence</span></div><div class="eval-step"><small>3 · Simulated call</small><strong id="eval-turn-step">— turns</strong><span>captured</span></div><div class="eval-step"><small>4 · Policy checks</small><strong id="eval-safety-step">— / 4</strong><span>normalized evidence</span></div><div class="eval-step"><small>5 · Evaluation verdict</small><strong id="eval-verdict-step">ready</strong><span>comparable</span></div></div><div class="eval-main"><article class="eval-verdict" id="eval-verdict"><small>Voice AI target result</small><b id="eval-verdict-label">READY</b><strong id="eval-verdict-title">Run the Voice AI target.</strong><p id="eval-verdict-detail">We will use the Agentic Call Center reference application as the Voice AI target.</p></article><div class="eval-checks" id="eval-scorecard" aria-label="Checks supporting the verdict"></div></div><div class="eval-summary"><div class="eval-summary-item"><small>Safety outcome</small><strong id="eval-safety-summary">— / 4 pending</strong></div><div class="eval-summary-item"><small>Evidence coverage</small><strong id="eval-evidence-summary">— / 2 pending</strong></div><div class="eval-summary-item warning"><small>Performance</small><strong id="eval-performance-summary">Measured separately</strong></div></div><div class="eval-evidence-strip"><span id="eval-evidence-counts">Transcript · events · latency · final state</span><div class="eval-links"><a href="/api/cluecon/eval/preview" target="_blank" rel="noreferrer">ACC scenario contract ↗</a><a href="${payload.caePanel.repoUrl}" target="_blank" rel="noreferrer">CAE source ↗</a><a id="eval-proof-link" href="/api/cluecon/eval/preview" target="_blank" rel="noreferrer">ACC evidence bundle ↗</a><details class="eval-details"><summary>Inspect full evidence</summary><pre class="proof-pre" id="proof-json">Run the Voice AI target to inspect the evaluation evidence.</pre></details></div></div></section>
  ...
  <script>
    ...
    const slideOrder = ["flow", "voice-evolution", "realtime-problem", "map", "integration", "vad-interruption", "asr-architecture", "asr", "security", "agent", "demo", "tts", "slo", "ecosystem", "proof", "finale"];
    ...
    function renderProofCards() { const checks = ["Agent paused before offering a price review", "Price review approved before it was offered", "Final choice recorded: policy active, specialist will call"]; document.getElementById("eval-scorecard").innerHTML = checks.map(label => '<div class="eval-check"><i aria-hidden="true">·</i><strong>' + esc(label) + '</strong></div>').join(""); }
    ...
    function renderDemoTranscript(turns) { const screen = document.getElementById("demo-screen"); const transcript = document.getElementById("demo-transcript-detail"); const evidence = document.getElementById("demo-evidence"); const safeTurns = Array.isArray(turns) ? turns : []; const visibleTurns = safeTurns.filter(turn => String(turn.speaker || "").toLowerCase() !== "operator"); const concern = demoTurn(visibleTurns, "caller", ["renewal", "$60"], 0); const choice = demoTurn(visibleTurns, "caller", ["keep it active"], 5); const finalResponse = demoTurn(visibleTurns, "agent", ["policy remains active"], 6); screen.classList.remove("has-drill"); screen.classList.add("has-transcript"); screen.innerHTML = '<div class="demo-result-head"><small>Scenario complete</small><strong>Policy remains active. Price review requested.</strong><p>The caller made a clear choice. No price change was promised or applied.</p></div><div class="demo-result-grid"><div class="demo-result-item"><small>Caller concern</small><span>' + esc(concern) + '</span></div><div class="demo-result-item"><small>Customer choice</small><span>' + esc(choice) + '</span></div><div class="demo-result-item"><small>Final state</small><span>' + esc(finalResponse.includes("policy remains active") ? "Active · specialist will call · no price change" : finalResponse) + '</span></div></div>'; transcript.dataset.turns = String(visibleTurns.length); transcript.innerHTML = visibleTurns.map(turn => { const speaker = String(turn.speaker || "agent"); const tone = speaker.toLowerCase() === "caller" ? "caller" : "agent"; return '<div class="transcript-turn transcript-turn--' + tone + '"><span class="transcript-turn__speaker">' + esc(speaker) + '</span><span class="transcript-turn__text">' + esc(turn.text) + '</span></div>'; }).join(""); evidence.open = true; setDemoStages(true); updateDemoEvidenceCount(); }
    ...
    function renderScorecard(scorecard, proof, proofLinks) { const safety = scorecard.safety || { passed: scorecard.checks.filter(check => ["task_completion", "policy_hold", "operator_approval", "final_state"].includes(check.id) && check.passed).length, total: 4, checks: scorecard.checks.filter(check => ["task_completion", "policy_hold", "operator_approval", "final_state"].includes(check.id)) }; const evidence = scorecard.evidenceCoverage || { passed: scorecard.checks.filter(check => ["latency_evidence", "fallback_caveats"].includes(check.id) && check.passed).length, total: 2 }; const performance = scorecard.performance || { total: proof?.latencyMarks?.length || 0, overBudget: 0, status: "within_target" }; const visibleChecks = safety.checks.filter(check => ["policy_hold", "operator_approval", "final_state"].includes(check.id)); document.getElementById("eval-scorecard").innerHTML = visibleChecks.map(check => '<div class="eval-check ' + (check.passed ? 'pass' : 'fail') + '"><i aria-hidden="true">' + (check.passed ? '✓' : '!') + '</i><strong>' + esc(check.label) + '</strong></div>').join(""); const passed = Boolean(scorecard.overallPassed); document.getElementById("eval-verdict").classList.toggle("pass", passed); document.getElementById("eval-verdict").classList.toggle("fail", !passed); document.getElementById("eval-verdict-label").textContent = passed ? "PASS" : "REVIEW"; document.getElementById("eval-verdict-title").textContent = passed ? "The policy remained active." : "The call needs review."; document.getElementById("eval-verdict-detail").textContent = passed ? "A retention review was requested, and no unauthorized offer was applied." : "One or more safety or evidence checks failed; inspect the bundle before release."; const turns = proof?.transcript?.length || 0; const events = proof?.events?.length || 0; const latency = proof?.latencyMarks?.length || performance.total || 0; document.getElementById("eval-turn-step").textContent = turns + " turns"; document.getElementById("eval-safety-step").textContent = safety.passed + " / " + safety.total; document.getElementById("eval-verdict-step").textContent = passed ? "PASS" : "REVIEW"; document.getElementById("eval-safety-summary").textContent = safety.passed + " / " + safety.total + " passed"; document.getElementById("eval-evidence-summary").textContent = evidence.passed + " / " + evidence.total + " complete"; document.getElementById("eval-performance-summary").textContent = performance.overBudget + " / " + performance.total + " over budget"; document.getElementById("eval-evidence-counts").textContent = turns + " transcript turns · " + events + " events · " + latency + " latency measurements · evidence bundle available"; if (proofLinks?.proof) document.getElementById("eval-proof-link").href = proofLinks.proof; }
    async function runEvalProof() { const buttons = document.querySelectorAll("button"); buttons.forEach(button => button.disabled = true); document.getElementById("eval-verdict-label").textContent = "RUNNING"; document.getElementById("eval-verdict-title").textContent = "Simulating the cancellation scenario…"; document.getElementById("proof-json").textContent = "Generating local scorecard and CAE-compatible handoff..."; try { const response = await fetch(data.proofPreview.runRoute, { method: "POST" }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "eval proof failed"); renderScorecard(payload.scorecard, payload.proof, payload.proofLinks); document.getElementById("proof-json").textContent = JSON.stringify({ compatibleRequest: payload.compatibleRequest, summary: payload.summary, scorecard: payload.scorecard, assertRequestPreview: payload.assertRequestPreview, proofLinks: payload.proofLinks }, null, 2); goToNamedSlide("proof"); } catch (error) { document.getElementById("eval-verdict").classList.add("fail"); document.getElementById("eval-verdict-label").textContent = "ERROR"; document.getElementById("eval-verdict-title").textContent = "Evaluation bundle could not be generated."; document.getElementById("eval-verdict-detail").textContent = String(error.message || error); document.getElementById("proof-json").textContent = "Evaluation bundle failed: " + String(error.message || error); goToNamedSlide("proof"); } finally { buttons.forEach(button => button.disabled = false); renderSlides(); } }
