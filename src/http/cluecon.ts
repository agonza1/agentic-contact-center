...
<body class="${bodyClass}">
  ...
  <script>
    ...
    function clearVadTurnTimers() { clearTimeout(state.vadTurnTimer); clearTimeout(state.vadOutputTimer); state.vadTurnTimer = null; state.vadOutputTimer = null; }
    ...
    function startVadBot() { if (state.vadBotSpeaking) { stopVadBot(false); return; } if (state.vadTurnTimer) { vadLog("OUTPUT_BLOCKED", "Agent audio cannot start while the 4 s end-of-turn gate is still pending."); vadStatus("turn wait: 4.0 s", "fixture"); return; } state.vadBotSpeaking = true; document.getElementById("vad-agent").textContent = "Stop agent speech"; vadStatus("agent speaking", "ready"); vadNodes(state.vad ? ["vad-node-input"] : [], "vad-node-output", null); vadLog("TTSStartedFrame", "Agent audio is flowing. Speak now to demonstrate barge-in."); const finish = () => stopVadBot(false); if ("speechSynthesis" in window && window.SpeechSynthesisUtterance) { window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance("I can review the available account options with you. First, let me confirm a few details before we continue."); utterance.rate = 0.82; utterance.onend = finish; utterance.onerror = finish; window.speechSynthesis.speak(utterance); state.vadBotTimer = setTimeout(finish, 12000); } else { state.vadBotTimer = setTimeout(finish, 8000); } }
    function vadSpeechStart(now) { const vad = state.vad; if (vad && vad.speaking) return; const wasWaitingForTurnEnd = Boolean(state.vadTurnTimer || state.vadOutputTimer); clearVadTurnTimers(); if (vad) { vad.speaking = true; vad.speechStartedAt = now; vad.aboveSince = null; vad.belowSince = null; } vadStatus("user speaking", "ready"); if (wasWaitingForTurnEnd) vadLog("TURN_WAIT_CANCELLED", "Caller resumed before the 4 s end-of-turn gate; audio output remains blocked."); vadLog("VADUserStartedSpeakingFrame", "Acoustic speech onset confirmed after 80 ms."); vadLog("UserStartedSpeakingFrame", "User turn start strategy opens the turn and enables interruption."); if (state.vadBotSpeaking) stopVadBot(true); else vadNodes(["vad-node-input", "vad-node-turn"], null, null); }
    function vadSpeechStop(now) { const vad = state.vad; if (vad && !vad.speaking && !state.vadTurnTimer) return; const duration = vad && vad.speechStartedAt ? Math.max(0, Math.round(now - vad.speechStartedAt)) : 780; clearVadTurnTimers(); if (vad) { vad.speaking = false; vad.aboveSince = null; vad.belowSince = null; } vadStatus("turn wait: 4.0 s", "fixture"); vadLog("VADUserStoppedSpeakingFrame", "Acoustic silence held for 350 ms; segment was " + duration + " ms."); vadLog("EndOfTurnCandidate", "Holding audio output until 4.0 s of post-speech silence is observed."); vadNodes(state.vad ? ["vad-node-input", "vad-node-turn", "vad-node-stt"] : ["vad-node-turn", "vad-node-stt"], null, null); state.vadTurnTimer = setTimeout(() => { state.vadTurnTimer = null; vadLog("UserStoppedSpeakingFrame", "4.0 s silence gate elapsed; user turn is now closed."); vadLog("EndOfTurnDetected", "Only now can the agent response enter the latency budget for first token and TTS."); vadStatus("end-of-turn", "ready"); vadNodes(state.vad ? ["vad-node-input", "vad-node-agent"] : ["vad-node-agent"], null, null); state.vadOutputTimer = setTimeout(() => { state.vadOutputTimer = null; vadLog("TTSStartedFrame", "Audio output starts after the 4 s end-of-turn gate."); vadStatus("agent response", "ready"); vadNodes(state.vad ? ["vad-node-input"] : [], "vad-node-output", null); setTimeout(() => { if (!state.vadBotSpeaking) { vadNodes(state.vad ? ["vad-node-input"] : [], null, null); vadStatus(state.vad ? "listening" : "mic off", state.vad ? "ready" : "fixture"); } }, 1200); }, 450); }, VAD_END_OF_TURN_MS); }
