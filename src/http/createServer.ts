...
const reliabilityVerdictEvidenceChecklist = [
  {
    id: "transcript",
    label: "Transcript",
...
const reliabilityValidationProvenance = {
  manifestPath: reliabilityProvenanceContract.manifestPath,
...
function selectedReliabilityModeEvidenceInventory(mode: string): typeof reliabilityEvidenceInventory {
...
function reliabilityEntrypointActionQueueSummary(
  ...
  const followUpIndex = followUpAction ? actionQueue.indexOf(followUpAction) : -1;
  ...
  return {
    ...
    followUpRunnable: followUpAction?.status === "pending",
    followUpBlocked: followUpAction?.status === "blocked",
    followUpComplete: followUpAction?.status === "complete",
    ...
    activeRequiresEvidence: Boolean(activeAction?.evidence),
...
function buildReliabilityTargetModes() {
  ...
  const missingEndpointEnvVars = (endpointKeys: Array<keyof typeof endpointRequirements>) =>
    endpointKeys.flatMap((key) => {
      const requirement = endpointRequirements[key];
  ...
  return [
    {
      ...
      validationGate: {
        ...
        provenance: reliabilityValidationProvenance,
      },
    ...
    },
    {
      ...
      nextAction: {
        step: browserLiveConfigured ? "validate_browser_media_path" : "configure_browser_media_endpoints",
        command: browserLiveConfigured ? "npm run browser-webrtc:check" : "npm run docker:browser-webrtc",
    ...
    },
    {
      ...
      nextAction: {
        step: sipVertoConfigured ? "validate_sip_verto_path" : "configure_sip_verto_endpoints",
        command: sipVertoConfigured ? "npm run pipecat:verto:check" : "npm run docker:sip-verto",
...
function buildReliabilityGuidePayload(config: PocConfig): object {
  ...
  return {
    ...
    comparisonContract: {
      ...
      verdictEvidenceChecklist: reliabilityVerdictEvidenceChecklist,
    },
