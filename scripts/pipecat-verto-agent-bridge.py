...
CALLER_IDENTITY_PARAM_KEY_PATTERN = re.compile(
    r"(caller[-_\s]?id|caller[-_\s]?(?:name|number|uri)|effective[-_\s]?caller|origination[-_\s]?caller|cid[-_\s]?(?:name|num|number)?|ani|clid|(?:variable[-_\s]?)?sip[-_\s]?(?:full[-_\s]?from|from[-_\s]?(?:uri|user|number|name)|h[-_\s]?(?:p[-_\s]?(?:asserted|preferred)[-_\s]?identity|remote[-_\s]?party[-_\s]?id|diversion|history[-_\s]?info))|p[-_\s]?(?:asserted|preferred)[-_\s]?identity|remote[-_\s]?party[-_\s]?id|diversion|history[-_\s]?info|full[-_\s]?from|from[-_\s]?(?:uri|user|number|name))",
    re.IGNORECASE,
)
